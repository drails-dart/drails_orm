...
/// + [nullable] - whether the column value nullable
...
class ColumnResult {
  ...
  const ColumnResult(String name) : this.name = name;
}

/// For class: specifies the discriminator column to distinguish different
/// classes in SINGLE_TABLE and/or JOINED inheritance mapping strategies.
///
...
/// For field: defines a composite foreign keys; groups of [joinColumn].
//@Target(value:"FIELD")
class JoinColumns {
  final List<JoinColumn> joinColumns;
...
/// + [uniqueConstraint] - uniqueConstrains placed on the table.
//@Target(value:"FIELD")
class JoinTable {
  final String name;
  final String catalog;
...
/// For field: specifies a column that is used to maintain the order of a list.
...
///  + [schema] - the database schema of the generator.
...
class UniqueConstraint {
  ...
  const UniqueConstraint(this.columnNames, {this.name});
}
